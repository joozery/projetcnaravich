import React from 'react';
import SilkSimple from './components/backgrounds/silk/SilkSimple';

function TestSilkSimple() {
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        
        <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">
          ทดสอบ Silk Component แบบง่าย
        </h1>

        {/* Silk พื้นฐาน */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">Silk พื้นฐาน</h2>
          <div className="h-64 rounded-lg overflow-hidden shadow-lg border-2 border-gray-300">
            <SilkSimple />
          </div>
        </div>

        {/* Silk แบบที่คุณต้องการ */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">Silk แบบที่คุณต้องการ</h2>
          <div className="h-64 rounded-lg overflow-hidden shadow-lg border-2 border-gray-300">
            <SilkSimple
              speed={5}
              scale={1}
              color="#7B7481"
              noiseIntensity={1.5}
              rotation={0}
            />
          </div>
        </div>

        {/* Silk สีน้ำเงิน */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">Silk สีน้ำเงิน</h2>
          <div className="h-64 rounded-lg overflow-hidden shadow-lg border-2 border-gray-300">
            <SilkSimple
              speed={3}
              scale={1.2}
              color="#2196F3"
              noiseIntensity={2}
              rotation={0.1}
            />
          </div>
        </div>

        {/* Silk สีเขียว */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">Silk สีเขียว</h2>
          <div className="h-64 rounded-lg overflow-hidden shadow-lg border-2 border-gray-300">
            <SilkSimple
              speed={8}
              scale={0.8}
              color="#4CAF50"
              noiseIntensity={1}
              rotation={-0.05}
            />
          </div>
        </div>

        {/* Grid ของ Silk ขนาดเล็ก */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">Silk ขนาดเล็ก (Grid)</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="h-32 rounded-lg overflow-hidden shadow border border-gray-300">
              <SilkSimple speed={4} color="#FF6B6B" scale={1.1} />
            </div>
            <div className="h-32 rounded-lg overflow-hidden shadow border border-gray-300">
              <SilkSimple speed={6} color="#4ECDC4" scale={0.9} />
            </div>
            <div className="h-32 rounded-lg overflow-hidden shadow border border-gray-300">
              <SilkSimple speed={3} color="#45B7D1" scale={1.3} />
            </div>
          </div>
        </div>

        {/* ข้อมูลการใช้งาน */}
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-bold mb-4 text-gray-800">วิธีการใช้งาน</h3>
          <div className="space-y-2 text-sm text-gray-600">
            <p><strong>speed:</strong> ความเร็วของการเคลื่อนไหว (1-10)</p>
            <p><strong>scale:</strong> ขนาดของเอฟเฟกต์ (0.5-2)</p>
            <p><strong>color:</strong> สีหลัก (hex format เช่น #7B7481)</p>
            <p><strong>noiseIntensity:</strong> ความเข้มของ noise (0.5-3)</p>
            <p><strong>rotation:</strong> การหมุนของ pattern (-0.5 ถึง 0.5)</p>
          </div>
          
          <div className="mt-4 p-4 bg-gray-100 rounded">
            <h4 className="font-bold mb-2">ตัวอย่างโค้ด:</h4>
            <pre className="text-xs text-gray-700">
{`import SilkSimple from './SilkSimple';

<SilkSimple
  speed={5}
  scale={1}
  color="#7B7481"
  noiseIntensity={1.5}
  rotation={0}
/>`}
            </pre>
          </div>
        </div>

        {/* แบนเนอร์ตัวอย่าง */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-gray-800">แบนเนอร์ตัวอย่าง</h2>
          <div className="relative h-64 rounded-xl overflow-hidden shadow-lg">
            <SilkSimple
              speed={2}
              scale={1.5}
              color="#2196F3"
              noiseIntensity={1.2}
              rotation={0.05}
            />
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center">
              <h1 className="text-3xl font-bold mb-2 drop-shadow-lg">ตลาดหนี้มือถือ</h1>
              <p className="text-lg mb-6 opacity-95">เลือกซื้อหนี้คุณภาพ ลงทุนอย่างชาญฉลาด</p>
              <div className="flex gap-8 text-lg">
                <div className="text-center">
                  <div className="text-2xl font-bold">4</div>
                  <div className="text-sm">รายการหนี้</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">100%</div>
                  <div className="text-sm">ข้อมูลจริง</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">24/7</div>
                  <div className="text-sm">บริการ</div>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>
  );
}

export default TestSilkSimple;




